            ...
            <form method="POST" action="{{ route('tickets.update', $ticket) }}">
                ...
                <div class="mb-4">
                    ...
                    <div class="row row-cols-1 g-2">
                        @foreach($ticket->attachments as $index => $attachment)
                            <div class="p-3 rounded-3 mb-2" id="existing-attachment-{{ $attachment->id }}" style="background-color: #f8fafc; border: 1px solid #e2e8f0;">
                                ...
                                <div class="mt-2.5">
                                    <label for="existing_note_{{ $attachment->id }}" class="form-label text-muted small fw-semibold mb-1">File Note (Optional)</label>
                                    <textarea id="existing_note_{{ $attachment->id }}" name="existing_notes[{{ $attachment->id }}]" class="form-control form-control-sm" rows="2" placeholder="Enter an optional note/description for this file...">{{ old('existing_notes.' . $attachment->id, $attachment->note) }}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="mb-4">
                    ...
                    <div id="drop-zone" class="border border-2 border-dashed rounded p-4 text-center bg-light" style="border-style: dashed !important; transition: background-color 0.2s, border-color 0.2s; cursor: pointer;">
                        ...
                        <p class="text-muted mb-0" style="font-size: 0.75rem;">Allowed formats: .webp, .pdf, .doc, .docx, .xls, .xlsx, .csv (Images will be automatically converted to WebP on the client side)</p>
                        <input type="file" id="file-input" class="d-none" multiple accept=".jpg,.jpeg,.jfif,.png,.gif,.webp,.pdf,.xls,.xlsx,.csv,.doc,.docx">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="department_id" class="form-select @error('department_id') is-invalid @enderror" name="department_id" disabled>
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" data-division-id="{{ $department->division_id }}" {{ old('department_id', $ticket->department_id) == $department->id ? 'selected' : '' }}>{{ $department->name }}</option>
                            @endforeach
                        </select>
                        @error('department_id')
                            <span class="invalid-feedback" role="alert">
...
<script type="module">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function filterDepartments(initial = false) {
            ...
            const previousVal = initial ? "{{ old('department_id', $ticket->department_id) }}" : departmentSelect.value;
            ...
            placeholderOption.textContent = selectedDivisionId ? 'Select Department' : 'Select Division First';
