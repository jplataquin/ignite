...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2 fw-bold text-dark">Ticket Priorities</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <a href="{{ route('admin.ticket-priorities.create') }}" class="btn btn-primary d-flex align-items-center">
    ...
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    <th scope="col" class="px-4 py-3 text-muted fw-bold text-uppercase small" style="width: 100px;">Level</th>
                    <th scope="col" class="py-3 text-muted fw-bold text-uppercase small">Name</th>
            ...
            <tbody>
                @forelse($priorities as $priority)
                    <tr>
                        <td class="px-4 py-3 fw-bold">
                            <span class="badge bg-secondary rounded-pill px-3 py-1.5 fw-semibold">{{ $priority->level }}</span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-end">
                            <div class="d-inline-flex gap-1">
                                <a href="{{ route('admin.ticket-priorities.edit', $priority) }}" class="btn btn-sm btn-outline-secondary d-flex align-items-center" style="min-height: 34px;">
                                    Edit
                                </a>
                                <form action="{{ route('admin.ticket-priorities.destroy', $priority) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this priority?');">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted">No priorities found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
